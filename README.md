...
Daily 001 は日本時間の2026年9月21日を基準に開始します。最初の100日分はあらかじめ決まった問題セットで、キー数・配置、行列サイズ、MCU位置とピン割り当て、ダイオード方向、キープアウトが日ごとに異なります。通常は当日分までしか開けません。特定の日を開くには`?puzzle=day001`、パス形式では`/day/001`を使います。判定、幾何計算、問題の読み込み、進行状況の保存は`src/lib`に分けています。
