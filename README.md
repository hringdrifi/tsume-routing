...
1. ダイオードは各スイッチの近くに配置済みです。ダイオードはドラッグして移動できます。スイッチまたはダイオードをクリックして選択し、盤面上部の回転ボタンかRキーで90度回転します。回転・移動した部品の端子は動きますが、既存の配線は動きません。
...
進行状況はブラウザ内に自動保存され、同じ問題を再読込すると復元されます。保存形式が古い場合は破棄して初期状態から始めます。

同じ層で配線が交差・接触すると導通します。異なる層はビアまたはスルーホールのMXピンを介して導通します。MCUの各ピンにはROW/COLの割り当てがチップ内に表示されます。未接続のパッド間にはラッツネストの点線が表示され、接続すると消えます。各スイッチのCOLパッドを対応するCOLピンへ、反対側のパッドを専用ダイオードのスイッチ側へ、ダイオードの残りのパッドを対応するROWピンへつないでください。ダイオードのスイッチ側は`COL2ROW`ならアノード、`ROW2COL`ならカソードです。ダイオード内部とスイッチ内部は検証時に短絡として扱いません。
...
基本点は100点です。未接続・ショートは各−10、キープアウト侵入は−5、配線長は1 mmごとに−0.1、ビアは各−1です。キープアウト侵入はクリアを妨げません。
...
Daily 001 は日本時間の2026年9月22日を基準に開始します。最初の100日分はあらかじめ決まった問題セットで、キー数・配置、行列サイズ、MCU位置とピン割り当て、ダイオード方向、キープアウトが日ごとに異なります。通常は当日分までしか開けません。特定の日を開くには`?puzzle=day001`、パス形式では`/day/001`を使います。公開前の問題は、確認用に`?debug=1&puzzle=day002`（または`/day/002?debug=1`）で開けます。判定、幾何計算、問題の読み込み、進行状況の保存は`src/lib`に分けています。
