...
Daily 001 は日本時間の2026年9月21日を基準に開始します。最初の10日分は固定の問題セットで、キー数・配置、行列サイズ、MCU位置とピン割り当て、ダイオード方向、キープアウトが日ごとに異なります。通常は当日分までしか開けません。公開前の問題は、確認用に`?debug=1&puzzle=day002`（または`/day/002?debug=1`）で開けます。判定、幾何計算、問題の読み込み、進行状況の保存は`src/lib`に分けています。
