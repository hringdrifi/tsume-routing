...
const puzzle=validatePuzzle({id:'test',title:'Test',board:{width:50,height:45},matrix:{rows:1,cols:1,diodeDirection:'COL2ROW'},switches:[{id:'SW1',x:12,y:12,rotation:90,row:0,col:0}],mcu:{x:37,y:25,pins:[{number:1,role:'ROW0'},{number:2,role:'COL0'}]},keepouts:[{id:'KO',type:'rect',x:19,y:30,width:5,height:5}],scoring:{base:100,connectionError:-10,keepoutViolation:-5,routeLength:-.1,via:-1}})
...
describe('electrical and scoring',()=>{
  ...
  it('uses rotated switch pins without an orientation penalty',()=>{const s=solved();const before=pads(puzzle,s).find(p=>p.id==='SW1:col')!;s.switchRotations.SW1=0;const after=pads(puzzle,s).find(p=>p.id==='SW1:col')!;expect(after).not.toEqual(before)})
  it('penalizes route length as well as vias and keepout intrusion',()=>{const s=solved();const nodes:RouteNode[]=[{x:20,y:32,layer:'F.Cu'},{x:20,y:32,layer:'B.Cu'},{x:24,y:32,layer:'B.Cu'}];s.traces.push({id:'via',nodes});const r=evaluate(puzzle,s);expect(r.viaCount).toBe(1);expect(r.keepout).toBeGreaterThan(0);expect(r.score).toBe(Math.round((100-r.keepout*5-r.length*.1-1)*10)/10)})
