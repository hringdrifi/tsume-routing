...
const key=(p:Puzzle)=>{const raw=JSON.stringify(p);let hash=2166136261;for(let i=0;i<raw.length;i++){hash^=raw.charCodeAt(i);hash=Math.imul(hash,16777619)}return `tsume-progress-v1:${p.id}:${hash>>>0}`}
export function loadProgress(p:Puzzle):BoardState|null{
  try{
    ...
    if(!Array.isArray(s.diodes)||s.diodes.length!==p.switches.length||!Array.isArray(s.traces)||s.traces.length>1000)return null
