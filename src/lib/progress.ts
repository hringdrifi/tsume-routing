...
const key=(p:Puzzle)=>{const raw=JSON.stringify(p);let hash=2166136261;for(let i=0;i<raw.length;i++){hash^=raw.charCodeAt(i);hash=Math.imul(hash,16777619)}return `tsume-progress-v3:${p.id}:${hash>>>0}`}
