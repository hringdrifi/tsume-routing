...
export function evaluate(p:Puzzle,state:BoardState):Result {
  ...
  const score=Math.round((p.scoring.base+(missing+shorts)*p.scoring.connectionError+keepout*p.scoring.keepoutViolation+length*p.scoring.routeLength+viaCount*p.scoring.via)*10)/10
  return {clear:missing===0&&shorts===0,score,viaCount,length,issues,missing,shorts,keepout}
}
