...
it('creates one hundred fixed, varied daily puzzles',()=>{
  ...
  expect(dailyPuzzle(100).switches).toHaveLength(12)
  expect(dailyPuzzle(100)).toEqual(dailyPuzzle(100))
...
it('keeps keepouts clear of every MCU pin pad',()=>{
  for(let day=1;day<=DAILY_COUNT;day++){
    const puzzle=dailyPuzzle(day)
    for(const [index,pin] of puzzle.mcu.pins.entries())for(const keepout of puzzle.keepouts){
      const x=puzzle.mcu.x-7,y=puzzle.mcu.y-8+index*4
      const overlaps=x+1.4>keepout.x&&x-1.4<keepout.x+keepout.width&&y+1.4>keepout.y&&y-1.4<keepout.y+keepout.height
      expect(overlaps,`day ${day}: MCU pin ${pin.number} overlaps ${keepout.id}`).toBe(false)
    }
  }
})
